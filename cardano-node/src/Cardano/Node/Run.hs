...
runNode loggingLayer npm = do
    ...
    (mscFp', configFp', genHash', traceOpts') <-
        case npm of
          MockProtocolMode (NodeMockCLI mMscFp genHash _ configYaml mTraceOpts) ->
            pure (mMscFp, configYaml, genHash, mTraceOpts)
          RealProtocolMode (NodeCLI rMscFp genHash _ configYaml rTraceOpts _) ->
            pure (rMscFp, configYaml, genHash, rTraceOpts)
...
handleSimpleNode p trace nodeTracers npm = do
  case npm of
    ...
    MockProtocolMode (NodeMockCLI mMscFp _ mockNodeAddr cfgYaml _) -> do
